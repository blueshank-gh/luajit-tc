...
function c12 (...)
  local x = {...}; local n = table.getn(x)
  local res = (n==2 and x[1] == 1 and x[2] == 2)
...
local call = function (f, args) return f(unpack(args, 1, #args)) end

assert(f() == 0)
assert(f({1,2,3}, 1, 2, 3) == 3)
assert(f({"alo", nil, 45, f, nil}, "alo", nil, 45, f, nil) == 4)
...
local a = vararg(call(next, {_G,nil}))
local b,c = next(_G)
assert(a[1] == b and a[2] == c and #a == 2)
a = vararg(call(call, {c12, {1,2}}))
assert(#a == 2 and a[1] == 55 and a[2] == 2)
a = call(print, {'+'})
assert(a == nil)

local t = {1, 10}
function t:f (...) return self[({...})[1]]+#({...}) end
